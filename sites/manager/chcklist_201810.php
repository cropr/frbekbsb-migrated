...
<Body>
	...
	<table class="table1" align='center' width='70%'>
		<tr>
			...
			<td><h1>Parties encodées pour l'ELO 2018-10</h1><h5>ChckList</h5></td>
			<td width='8%'><a href='http://www.frbe-kbsb.be/'><img width=60 height=80 alt='FRBE' src='logos/Logo FRBE.png'></a></td>
		</tr>
		</table>
	<br>
	<form action="chcklist_201810.php" method="post">					
		...
		if (! EMPTY($_POST['nom']))
			{
				...
				{
					$lien='<a href="chcklist_201810.php?matlien='.$donnees['matricule'].'">'.$donnees['matricule'].'</a>';
					print "\t<tr>\n";
...
// --------------------------------------------------
		...
		$result = mysqli_query($fpdb,'SELECT COUNT(*) AS nombre FROM p_chcklist201810 WHERE Joueur = "'.$matricule.'"');
		$donnees = mysqli_fetch_array($result);
		$nombre =$donnees['nombre'];	
			
		$result = mysqli_query($fpdb,'SELECT p_chcklist201810.* FROM p_chcklist201810 WHERE p_chcklist201810.Joueur="'.$matricule.'" ORDER BY p_chcklist201810.PartieNr');
		...
		if (! empty($matricule)){
			if ($nombre>0){
				...
				<table class="table1" align="center">
					...
					while ($donnees = mysqli_fetch_array($result))
					{
						$result1 = mysqli_query($fpdb,'SELECT NomPrenom FROM p_player201807 WHERE Matricule="'.$donnees['Adversaire'].'"');
